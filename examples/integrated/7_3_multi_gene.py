...
# -*- coding: utf-8 -*-
"""
7_3: 多基因多组学整合可视化(基于 7_1 + l_2 多 gene 能力)

功能:
  - 19 gene 合并 GTF + ATAC/H3K27ac/CTCF/RNA × hiPSC_nor/hiPSC_var
  - 区域:chr17:75400000-76340000(7_× 统一范围)
  - 比较组:hiPSC_var--hiPSC_nor
  - 所有参数硬编码,直接 python 7_3_multi_gene.py 跑(不需要 CLI)

7_3 跟 7_1 的关系:
  - 7_3 = 7_1 换 region + 改单 gene → 多 gene
  - 其他完全照搬 7_1(TRACK_YLIM / 8 条 BW 注入 / 图形参数)

参考:
  - 7_1_multi_omics_integrated.py(基础模板)
  - l_2_multi_gene_visualization.py L212-241 (extract_gene_from_full_gtf), L350-366 (merge_gtf_files)
"""
...
# T-9.14: 让 python 7_3_multi_gene.py 直接跑(不需要 PYTHONPATH)
...
# 7_3: 19 个独立 GTF(每 gene 一个文件),不再发布 19_genes_subset.gtf(2.0 MB 冗余)
# 包含 19 gene: TMEM94 / CASKIN2 / TSEN54 / LLGL2 / RECQL5 / SAP30BP / ITGB4 / GALK1 / UNK / UNC13D / WBP2 / TRIM65 / FBF1 / ACOX1 / SRP68 / GALR2 / FOXJ1 / RNF157 / UBALD2
# FULL_GTF_FILE 是 fallback:如果某 gene 独立 .gtf 不存在,从这里捞。已发布 19 个独立 .gtf,所以 fallback 永远不触发
FULL_GTF_FILE = DEMO_DATA / "19_genes_subset.gtf"  # fall-back 路径(实际不会读)
...
# GTF 目录(已经按基因切好)
GENE_GTF_DIR = DEMO_DATA  # 19 个独立 .gtf + enhancer.bed 都放在这里
...
OUTPUT_DIR = Path(__file__).resolve().parent.parent.parent / "demo" / "output" / "7_3_multi_gene"
...
def extract_gene_from_full_gtf(gene_name: str) -> str:
    """从全 GTF 提取指定 gene 并保存为 {gene_name}.gtf(缺失时自动补)
    移植自 l_2 L212-241。
    """
    gene_gtf_path = GENE_GTF_DIR / f"{gene_name}.gtf"
    if gene_gtf_path.exists():
        return str(gene_gtf_path)

    if not FULL_GTF_FILE.exists():
        print(f"  [警告] 全 GTF 文件不存在: {FULL_GTF_FILE}")
        return None

    GENE_GTF_DIR.mkdir(parents=True, exist_ok=True)
    gene_lines = []
    try:
        with open(FULL_GTF_FILE, "r") as f:
            for line in f:
                if line.startswith("#"):
                    continue
                parts = line.strip().split("\t")
        ...
        if gene_lines:
            with open(gene_gtf_path, "w") as f:
                f.write("\n".join(gene_lines) + "\n")
            print(f"  [OK] 从全 GTF 提取: {gene_name} -> {gene_gtf_path}")
            return str(gene_gtf_path)
    except Exception as e:
        print(f"  [错误] 提取 gene '{gene_name}' 失败: {e}")
    return None


def merge_gtf_files(gene_names: list, output_path: str) -> bool:
    """合并多个基因的 GTF 文件(同 l_2)
    7_3 增强:单个 gene GTF 缺失时 fallback 到 extract_gene_from_full_gtf
    """
    all_lines = []
    for gene_name in gene_names:
        gtf_path = get_gene_gtf_path(gene_name)
        if not gtf_path:
            # 7_3: 单 gene GTF 缺失,从全 GTF 提取
            gtf_path = extract_gene_from_full_gtf(gene_name)
    ...
    for i, sample in enumerate(samples["hic"]):
        ...
        if hic_path:
            hics.append({
                ...
                "balance": True,
                "name": sample,
                "flip_vertical": (i == 1),  # D11 翻转(跟 7_1 一致)
            })
...
if __name__ == "__main__":
    # 7_3: 硬编码参数(用户直接 python 7_3_multi_gene.py 跑)
    GENES = [
        "TMEM94", "CASKIN2", "TSEN54", "LLGL2", "RECQL5", "SAP30BP",
        "ITGB4", "GALK1", "UNK", "UNC13D", "WBP2", "TRIM65", "FBF1",
        "ACOX1", "SRP68", "GALR2", "FOXJ1", "RNF157", "UBALD2",
    ]
    REGION = "chr17:75400000-76340000"  # T-9.14: 保持原值(已是 7_× 统一范围)
    COMPARISON = "hiPSC_var--hiPSC_nor"  # T-9.14: 跟 sample name + demo 配置同步

    # 输出目录(T-9.14: 相对 demo/output,用顶部 OUTPUT_DIR)
    output_dir = OUTPUT_DIR
    output_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print("7_3: 多基因多组学整合可视化(基于 7_1 + l_2)")
    print("=" * 60)
    print(f"基因列表: {GENES}")
    print(f"绘图区域: {REGION}")
    print(f"比较组: {COMPARISON}")
    ...
    # 调主可视化函数(7_1 风格的 run_multi_gene_visualization)
