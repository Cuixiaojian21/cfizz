...
def calculate_sample_stats(pairing_df: pd.DataFrame, sample_name: str) -> dict:
    ...
    pairing_df : pd.DataFrame
        包含 is_stable 字段的 DataFrame (不需要 quality_group)
    sample_name : str
        样本名称
    ...
    -------
    dict
        {sample, total, Stable, Unique, Stable_pct, Unique_pct}
    """
    ...
    stats = {
        'sample': sample_name,
        'total': total,
        'Stable': int(sample_data['is_stable'].sum()) if total > 0 else 0,
        'Unique': int((~sample_data['is_stable']).sum()) if total > 0 else 0,
    }

    stats['Stable_pct'] = stats['Stable'] / total * 100 if total > 0 else 0
    stats['Unique_pct'] = stats['Unique'] / total * 100 if total > 0 else 0
...
def load_plot_data_loops(output_path: Path) -> dict:
    ...
    for _, row in plot_data.iterrows():
        ...
        data_dict[sample] = {
            ...
            'Unique_pct': float(row['Unique_pct']),
        }
...
def plot_loops_stacked_bar(data_dict: dict, comparison: str, output_dir: Path) -> None:
    ...
    categories = ['Stable', 'Unique']
    category_labels = ['stable (matched)', 'unique (unmatched)']
...
def analyze_single_comparison_loops(
    ...
    """
    分析单个比较组的Loops差异（双向独立分析）。

    双向独立 = 调两次 find_matching_loops:
      - 第一次: var 建树, 遍历 nor -> nor 视角产物
      - 第二次: nor 建树, 遍历 var -> var 视角产物

    产物文件 (8个独立, 2套视角各自描述):
      - nor_pairing.tsv / var_pairing.tsv  (含 partner_key + bin)
      - 1_1_nor_stable_loops.tsv / 1_1_nor_unique_loops.tsv
      - 1_1_var_stable_loops.tsv / 1_1_var_unique_loops.tsv
      - loops_plot_data.tsv (2 行, nor + var)
      - loops_<comparison>_stacked_bar.png/svg (二分类堆叠柱状图)
    """
    ...
    logger.info("=" * 70)
    logger.info(f"Loops差异分析 (双向独立): {comparison}")
    ...
    try:
        if run_mode in ['all', 'compute']:
            ...
            logger.info(f"    {control} loops: {len(loops_control)}")
            logger.info(f"    {treatment} loops: {len(loops_treatment)}")

            logger.info(f"\n2. 双向独立配对 - {control} 视角 (用 {treatment} 建树, 遍历 {control})")
            match_nor = find_matching_loops(
                loops_control, loops_treatment, control, treatment, distance_threshold=5
            )
            nor_view = match_nor['loops1_df'].copy()  # nor 视角全集
            logger.info(f"    {control} Stable: {nor_view['is_stable'].sum()} / {len(nor_view)}")

            logger.info(f"\n3. 双向独立配对 - {treatment} 视角 (用 {control} 建树, 遍历 {treatment})")
            match_var = find_matching_loops(
                loops_treatment, loops_control, treatment, control, distance_threshold=5
            )
            var_view = match_var['loops1_df'].copy()  # var 视角全集
            logger.info(f"    {treatment} Stable: {var_view['is_stable'].sum()} / {len(var_view)}")

            logger.info(f"\n4. 落盘 {control} 视角产物")
            pairing_cols = ['loop_key', 'bin1', 'bin2',
                            'partner_key', 'partner_bin1', 'partner_bin2',
                            'distance_bins', 'is_stable']

            # 构造 partner 三列(从 partner loops 查表)
            def _lookup_partner(view_df, partner_df):
                """根据 matched_idx2 查 partner_df 的 loop_key/bin1/bin2"""
                # partner_df 是 load_loops_data 读出的, 缺少 bin1/bin2/loop_key 列, 现算
                if 'loop_key' not in partner_df.columns:
                    partner_df = partner_df.copy()
                    partner_df['bin1'] = (partner_df['start1'] // 10000).astype(int)
                    partner_df['bin2'] = (partner_df['start2'] // 10000).astype(int)
                    swap_mask = partner_df['bin1'] > partner_df['bin2']
                    partner_df.loc[swap_mask, ['bin1', 'bin2']] = (
                        partner_df.loc[swap_mask, ['bin2', 'bin1']].values)
                    partner_df['loop_key'] = (
                        partner_df['chrom1'] + '_' +
                        partner_df['bin1'].astype(str) + '_' +
                        partner_df['bin2'].astype(str))
                idx_series = view_df['matched_idx2']
                out = pd.DataFrame({
                    'partner_key': idx_series.map(
                        lambda i: partner_df.loc[i, 'loop_key'] if pd.notna(i) else None),
                    'partner_bin1': idx_series.map(
                        lambda i: partner_df.loc[i, 'bin1'] if pd.notna(i) else None),
                    'partner_bin2': idx_series.map(
                        lambda i: partner_df.loc[i, 'bin2'] if pd.notna(i) else None),
                }, index=view_df.index)
                return out

            nor_partner = _lookup_partner(nor_view, loops_treatment)
            nor_pairing_out = pd.concat([
                nor_view[['loop_key', 'bin1', 'bin2', 'min_distance', 'is_stable']],
                nor_partner,
            ], axis=1).rename(columns={'min_distance': 'distance_bins'})[pairing_cols]

            var_partner = _lookup_partner(var_view, loops_control)
            var_pairing_out = pd.concat([
                var_view[['loop_key', 'bin1', 'bin2', 'min_distance', 'is_stable']],
                var_partner,
            ], axis=1).rename(columns={'min_distance': 'distance_bins'})[pairing_cols]

            nor_pairing_out.to_csv(
                comparison_dir / f'{control}_pairing.tsv', sep='\t', index=False)
            nor_view[nor_view['is_stable']].to_csv(
                comparison_dir / f'1_1_{control}_stable_loops.tsv', sep='\t', index=False)
            (~nor_view['is_stable']).pipe(lambda mask: nor_view[mask]).to_csv(
                comparison_dir / f'1_1_{control}_unique_loops.tsv', sep='\t', index=False)

            logger.info(f"    {control}_pairing.tsv: {len(nor_view)} 行 (8 列精简格式)")
            logger.info(f"    1_1_{control}_stable_loops.tsv: {nor_view['is_stable'].sum()} 行")
            logger.info(f"    1_1_{control}_unique_loops.tsv: {(~nor_view['is_stable']).sum()} 行")

            logger.info(f"\n5. 落盘 {treatment} 视角产物")
            var_pairing_out.to_csv(
                comparison_dir / f'{treatment}_pairing.tsv', sep='\t', index=False)
            var_view[var_view['is_stable']].to_csv(
                comparison_dir / f'1_1_{treatment}_stable_loops.tsv', sep='\t', index=False)
            (~var_view['is_stable']).pipe(lambda mask: var_view[mask]).to_csv(
                comparison_dir / f'1_1_{treatment}_unique_loops.tsv', sep='\t', index=False)

            logger.info(f"    {treatment}_pairing.tsv: {len(var_view)} 行")
            logger.info(f"    1_1_{treatment}_stable_loops.tsv: {var_view['is_stable'].sum()} 行")
            logger.info(f"    1_1_{treatment}_unique_loops.tsv: {(~var_view['is_stable']).sum()} 行")

            logger.info(f"\n6. 保存绘图数据 (两套结果不整合)")
            stats_list = [
                calculate_sample_stats(nor_view.assign(loop_source=control), control),
                calculate_sample_stats(var_view.assign(loop_source=treatment), treatment),
            ]
            save_plot_data_loops(stats_list, comparison_dir)
            logger.info(f"    loops_plot_data.tsv: 2 行 ({control} + {treatment})")
